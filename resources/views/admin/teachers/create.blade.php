    ...
    <form action="{{ route('admin.teachers.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="card-body">
            ...
            <div class="form-row">
                ...
                <div class="form-group" id="taught_classes_group" style="display: {{ in_array(old('position'), ['Guru Mata Pelajaran', 'Wali Kelas']) || !old('position') ? 'block' : 'none' }};">
                    ...
                    <select id="taught_classes" name="taught_classes[]" class="form-control form-select @error('taught_classes') is-invalid @enderror" multiple style="height: 120px;">
                        @foreach($classrooms as $room)
                            <option value="{{ $room->id }}" data-level="{{ $room->level }}" {{ in_array($room->id, old('taught_classes', [])) ? 'selected' : '' }}>{{ $room->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group" id="taught_subjects_group" style="display: {{ in_array(old('position'), ['Guru Mata Pelajaran', 'Wali Kelas']) || !old('position') ? 'block' : 'none' }};">
                    ...
                    <select id="taught_subjects" name="taught_subjects[]" class="form-control form-select @error('taught_subjects') is-invalid @enderror" multiple style="height: 120px;">
                        @foreach($subjects as $subject)
                            <option value="{{ $subject->id }}" data-level="{{ $subject->level }}" {{ in_array($subject->id, old('taught_subjects', [])) ? 'selected' : '' }}>{{ $subject->name }} (Kelas {{ $subject->level }})</option>
                        @endforeach
                    </select>
