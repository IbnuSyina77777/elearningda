    ...
    <form action="{{ route('admin.announcements.update', $announcement->id) }}" method="POST">
                ...
                <div class="col-md-6">
                    <div class="form-group mb-3">
                        ...
                        <select name="target_audience" class="form-control form-select @error('target_audience') is-invalid @enderror" required>
                            <option value="all" {{ old('target_audience', $announcement->target_audience) == 'all' ? 'selected' : '' }}>Semua (Guru & Siswa)</option>
                            <option value="teachers" {{ old('target_audience', $announcement->target_audience) == 'teachers' ? 'selected' : '' }}>Hanya Guru</option>
                            <option value="students" {{ old('target_audience', $announcement->target_audience) == 'students' ? 'selected' : '' }}>Hanya Siswa</option>
                        </select>
                ...
                <div class="col-md-6 d-flex align-center">
                    <div class="form-group mb-0" style="margin-top: 15px;">
                        <label class="d-flex align-center gap-sm cursor-pointer">
                            ...
                            <span style="font-weight:600;">Status Aktif (Ditampilkan)</span>
                        </label>
                        <div class="text-sm text-muted ml-4">Hapus centang untuk menarik (menyembunyikan) pengumuman.</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card-footer d-flex justify-between">
            ...
            <button type="submit" class="btn btn-primary">
                <i class="ri-save-line"></i> Perbarui Pengumuman
            </button>
        </div>
    </form>
</div>
